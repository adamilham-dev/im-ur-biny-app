...
Target perangkat utama: **tablet/iPad landscape 11" inch** sebagai display interaktif, namun UI responsif hingga ke layar ponsel.

> **Arsitektur AI: HYBRID (jujur).** Jalur utama saat ini memakai **Vision‑LLM Gemma via OpenRouter (cloud)** untuk mendeteksi + mengklasifikasi sampah (butuh internet). Model **on‑device TFLite** tetap di-bundle sebagai **fallback offline**. Detektor on‑device **RT‑DETR saat ini gagal dimuat** di perangkat (lihat *Catatan & Keterbatasan*). Jadi: bukan murni on‑device — ini hybrid cloud‑first dengan cadangan on‑device.

---

## Fitur Utama

- **Klasifikasi sampah via Vision‑LLM (Gemma/OpenRouter)** — satu objek (*single*) maupun banyak objek sekaligus (*mixed*) dengan pemetaan koordinat bounding box.
- **Fallback on‑device (TFLite)** — bila cloud tidak terkonfigurasi/offline, classifier on‑device mengambil alih (kategori `residu` ditekan agar konsisten).
- **Mode *mixed* + carousel hasil** — Gemma mengembalikan daftar objek + koordinat; UI menampilkan slide *global* (scene + semua kotak) lalu slide per‑objek (crop tiap objek).
...
|----------|--------|------------|
| **Kaca** | Botol, pecahan | Tempat daur ulang kaca |
...
> **Catatan tentang `Residu`:** kategori `residu` **sengaja dihilangkan** dari hasil. Prompt Gemma hanya menawarkan 5 kategori di atas; bila tidak yakin → `lainnya`. Model TFLite on‑device masih punya 6 output (termasuk residu) karena dilatih demikian, tetapi output residu dipetakan ke `lainnya` di kode (`_suppressResidu`). Enum internal `WasteCategory` di-serialisasi memakai **string id stabil** (`WasteCategory.stableId`), bukan index, agar tahan terhadap perubahan urutan.
...
| AI cloud (opsional) | **google_generative_ai** `^0.4.6` (Gemini — jalur escalation lama) |
...
├── app.dart                  # TrashScanApp + GoRouter (semua rute)
├── core/
...
3. Hasil dipetakan ke `ScanResult` (kategori + confidence + bounding box + crop). `residu` tidak pernah muncul (di luar daftar prompt).
...
- `TFLiteService.classifyImage()` mengklasifikasi gambar (atau center‑crop) on‑device; output `residu` → `lainnya`.
...
> **Penting (untuk paper):** Vision‑LLM (Gemma) **adalah** komponen AI cloud yang sekarang dipakai — jadi jangan klaim "murni on‑device / tanpa cloud / tanpa LLM". Deskripsi jujur: *hybrid* (cloud‑first Vision‑LLM + cadangan TFLite on‑device).

...
class DetailItemScreen extends ConsumerWidget {
      ...
      children: [
        Expanded(
          child: _buildStatCard(
            'CONFIDENCE',
            '$confPct%',
            'assets/images/page_10/Confidence.svg',
        ...
        Expanded(
          child: _buildStatCard(
            'XP DIDAPAT',
            '+$xp',
            'assets/images/page_10/XPStar.svg',
  ...
  Widget _buildStatCard(String label, String value, dynamic icon, Color bg,
          ...
          Container(
            ...
            child: Center(
              child: icon is String
                  ? SvgPicture.asset(icon, width: iconSz, height: iconSz)
                  : Icon(icon as IconData, size: iconSz, color: fg),
            ),
          ),
            ...
            child: Row(
              ...
              children: [
                Container(
                  ...
                  child: Center(
                    child: _buildStepIcon(step, iconSz, AppColors.textPrimary),
                  ),
                ),
                SizedBox(width: innerGap),
                Expanded(
  ...
  Widget _buildStepIcon(DisposalStep step, double iconSz, Color color) {
    final title = step.title.toLowerCase();

    if (title.contains('bilas') || title.contains('kosongkan')) {
      return SvgPicture.asset(
        'assets/images/page_10/Water.svg',
        width: iconSz,
        height: iconSz,
        colorFilter: ColorFilter.mode(color, BlendMode.srcIn),
      );
    } else if (title.contains('masukkan') || title.contains('buang')) {
      return SvgPicture.asset(
        'assets/images/page_10/Trash.svg',
        width: iconSz,
        height: iconSz,
        colorFilter: ColorFilter.mode(color, BlendMode.srcIn),
      );
    } else if (title.contains('lipat') || title.contains('ratakan')) {
      return Icon(Icons.layers_outlined, size: iconSz, color: color);
    } else if (title.contains('tutup')) {
      return Icon(Icons.restore_from_trash_rounded, size: iconSz, color: color);
    } else if (title.contains('bungkus')) {
      return Icon(Icons.inventory_2_outlined, size: iconSz, color: color);
    } else if (title.contains('pastikan') || title.contains('periksa')) {
      return Icon(Icons.fact_check_outlined, size: iconSz, color: color);
    }
    
    return Icon(Icons.check_circle_outline_rounded, size: iconSz, color: color);
  }

...
/// Pixel-perfect "18 · Manual Correction"
  ...
  Widget _buildGrid(Size size, int cols, WasteCategory? aiCategory,
    ...
    for (var r = 0; r < _categoryOrder.length; r += cols) {
      ...
      for (var c = 0; c < cols; c++) {
        ...
        // width each instead of leaving the 6th slot empty.
        rowChildren.add(
          Expanded(
            child: isMissing
  ...
  Widget _buildTopBar(BuildContext context, Size size,
    ...
    return Row(
      ...
      children: [
        ...
        GestureDetector(
          onTap: () {
            // Return to whichever screen launched manual correction
            // (passed via GoRouter `extra`), falling back to /result.
            final returnTo =
                GoRouterState.of(context).extra as String? ?? '/result';
            context.go(returnTo);
          },
          child: Container(
            width: isPhone ? 44 : 54,
            ...
            child: InkWell(
              ...
              onTap: _selectedCategory != null
                  ? () async {
                      ...
                      // /multi-result instead of the single-mode /feedback.
                      final reanalyzeIndex =
                          ref.read(reanalyzeMultiIndexProvider);
                      if (reanalyzeIndex != null) {
                        final corrected = ref.read(scanResultProvider);
                        if (corrected != null) {
                          ref.read(scanProvider.notifier).updateMultiResult(
                                ...
                                corrected.copyWith(
                                  confidence: 1.0,
                                  allProbabilities: {
                                    corrected.category.name: 1.0,
                                  },
                                ),
                              );
                        }
                        ref
                            .read(reanalyzeMultiIndexProvider.notifier)
                            .state = null;

...
/// Pixel-perfect match to Figma 385:1990 "14 · Conclusion — Kategori Existing"
...
///   - "VIA AI" badge top:14 (purple-tinted)
///   - tag bottom: "Kategori · 0.XX" (confidence in muted gray #C2CBDB)
/// - Right: 520px column, gap:24
///   1. Biny(84×88.2) + speech bubble "Aha, sekarang aku tahu!"
///   2. Category section: label + name + "DIKENALI LEWAT AI" purple pill
///      + description
...
class ConclusionExistingScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<ConclusionExistingScreen> createState() =>
      _ConclusionExistingScreenState();
}
...
class _ConclusionExistingScreenState extends ConsumerState<ConclusionExistingScreen> {
  ...
  Widget _buildPortrait(BuildContext context, Size size, ScanResult result,
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        children: [
          SizedBox(
            height: isPhone ? size.shortestSide * 0.75 : size.shortestSide * 0.5,
            width: isPhone ? size.shortestSide * 0.75 : size.shortestSide * 0.5,
            child: _buildImageCard(result, capturedImage, isPhone: isPhone, scale: scale),
          ),
          SizedBox(height: isPhone ? 20 : 32),
          _buildRightPanel(context, size, result, ref, isPortrait: true, scale: scale),
        ],
      ),
    );
  }
        ...
        child: Container(
          ...
          child: Stack(
            ...
            children: [
              ...
              // "VIA AI" badge — Figma: top:14.4
              // Purple-tinted regardless of category.
              Positioned(
                top: 14 * scale,
                left: 0,
                right: 0,
              ...
              // Confidence value in muted gray #C2CBDB (Figma).
              Positioned(
                bottom: 14 * scale,
                left: 0,
                right: 0,
  ...
  Widget _buildRightPanel(BuildContext context, Size size, ScanResult result,
    ...
    return Column(
      ...
      children: [
        ...
        // 3. Dataset chip
        _buildDatasetChip(size, scale: scale, isPhone: isPhone),
        SizedBox(height: isPortrait ? gap16 : gap24),
  ...
  Widget _buildBinyBubble(Size size,
    ...
    return Row(
      ...
      children: [
        BinyHero(
            size: mascotSize, expression: BinyExpression.conclusionExisting),
  ...
  // + "DIKENALI LEWAT AI" solid purple pill + description 17px Medium
  // ─────────────────────────────────────────────────────────────────────
      ...
      children: [
        ...
        Row(
          children: [
            ...
            SizedBox(width: 16 * scale),
            // "DIKENALI LEWAT AI" pill — solid purple (not gradient).
            Container(
              padding: EdgeInsets.symmetric(
                horizontal: 16 * scale,
                vertical: 9 * scale,
  ...
  // Icon box uses purple soft bg (NOT category-tinted) — matches the
  // "VIA AI / existing" branding.
  // ─────────────────────────────────────────────────────────────────────
  Widget _buildDatasetChip(Size size,
      {double scale = 1.0, bool isPhone = false}) {
    ...
    return Container(
      ...
      child: Row(
        children: [
          // Icon box — purple soft bg, folder-check icon
          Container(
            width: iconBox,
            height: iconBox,
            decoration: BoxDecoration(
          ...
          Expanded(
            child: Column(
              ...
              children: [
                Text(
                  'Cocok dengan kategori yang ada',
                  style: GoogleFonts.baloo2(
                    fontSize: titleSize,
                    fontWeight: FontWeight.w700,
                    color: AppColors.textPrimary,
          ...
          Container(
            ...
            child: Text(
              '+${SessionService.xpExistingCategory} XP',
              style: GoogleFonts.baloo2(
                fontSize: xpSize,
                fontWeight: FontWeight.w800,
  ...
  // "Koreksi"(#EDE8FF, drop-shadow #DDD3FF) + "Selesai"(#7C5CFC)
  // ─────────────────────────────────────────────────────────────────────
  Widget _buildButtonsRow(BuildContext context, WidgetRef ref, Size size,
    ...
    return Row(
      children: [
        ...
        SizedBox(width: gap),
        Expanded(
          child: _buildSelesaiButton(context, ref, size, scale: scale),
        ),
      ],
    );
  }
  ...
  /// "Selesai" — save to history + award XP + add scan, go to /dataset-saved.
  Widget _buildSelesaiButton(BuildContext context, WidgetRef ref, Size size,
      {double scale = 1.0}) {
    final isPhone = AppResponsive.isPhone(size);
    final padH = (isPhone ? 16.0 : 32.0) * scale;
    final padV = (isPhone ? 14.0 : 20.0) * scale;

...
class CameraGuideNotifier extends StateNotifier<CameraGuideState> {
  ...
  ///
  /// Timer cancellation is synchronous, but provider state mutations are
  /// deferred to a microtask so they never fire inside Flutter's build /
  /// dispose phase (which would throw StateNotifierListenerError).
  void stopValidation() {
    _timer?.cancel();
    _timer = null;
    _frameCount = 0;
...
final cameraGuideProvider =
  ...
  // Always share the single CameraService owned by cameraProvider — never
  // spin up a second controller that would never be initialized.
  final cameraService = ref.read(cameraProvider.notifier).service!;
  return CameraGuideNotifier(cameraService, ref);
});
